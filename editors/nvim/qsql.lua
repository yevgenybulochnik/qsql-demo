-- quicksql language server client for Neovim — native vim.lsp, no plugins required.
--
-- Load it directly:            :luafile /path/to/editors/nvim/qsql.lua
-- or from init.lua (if placed on your runtimepath as lua/qsql.lua):
--                              require("qsql")
--
-- The server must be on PATH:  uv tool install 'quicksql[lsp]'
--                              (or: pipx install 'quicksql[lsp]')
-- so that `quicksql lsp` runs. Diagnostics publish on open/change; column completion
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(args)
    ...
    -- a quicksqlrc.py or base.qsql marks the project root; else the file's dir
    local marker = vim.fs.find({ "quicksqlrc.py", "base.qsql" }, { upward = true, path = dir })[1]
...
--       cmd = { "quicksql", "lsp" },
--       filetypes = { "qsql" },
--       root_dir = require("lspconfig.util").root_pattern("quicksqlrc.py", "base.qsql"),
--     },
--   }
