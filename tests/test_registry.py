...
from qsql_demo.registry import PLUGINS, PluginRegistry, plugin


def test_builtins_are_registered(registries) -> None:
    for name in ("engine", "input", "sources", "extensions", "output", "autorun", "vars", "depends_on", "tags"):
        assert name in PLUGINS
        assert PLUGINS.field_owner(name) is PLUGINS.get(name)
